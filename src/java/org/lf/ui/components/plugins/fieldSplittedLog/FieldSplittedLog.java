...
public class FieldSplittedLog extends JPanel{
	private final FieldValuesListModel listModel = new FieldValuesListModel();
	private JList fieldValues;
	private final LogAndField logAndField;
	private JSplitPane splitPane;
	private JPanel defaultPanel = new JPanel(new BorderLayout());

	public FieldSplittedLog(LogAndField logAndField){
				...
				if(e.getButton() == MouseEvent.BUTTON1 ){
					...
					if (e.getClickCount() == 2) {
						if (panel == null) {
							...
							panel = new ScrollableLogTable(log);
