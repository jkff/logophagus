...
public class MergeLogs implements Log {
	...
	private class MergedRecord implements Record {
		MergedPosition pos;
		int size = 0;
		private Map<Integer, Integer> indexTransformation;
		...
		public String get(int index) {
			if (!getIndexTransformation().containsKey(index)) return null;
			CurPrevIndex entity = pos.curSortedSet.first();
			return entity.first.second.get(getIndexTransformation().get(index));

		}

		//key is the index of merged record and value is the index of underlying record 
		private Map<Integer, Integer> getIndexTransformation() {
			if (indexTransformation != null) return indexTransformation; 
			indexTransformation = new HashMap<Integer, Integer>();
			CurPrevIndex entity = pos.curSortedSet.first();
			if (entity.third.intValue() == 0) { 
				for (int i =0 ; i < entity.first.second.size() ; ++i ) {
						indexTransformation.put(i, i);
				}
			} else {
				int lengthSum = 0;
				...
				indexTransformation.put(timeFieldIndices[0], timeFieldIndices[entity.third]);
				int indexShift = lengthSum - entity.third.intValue() + 1; 
				for (int i =0 ; i < entity.first.second.size() ; ++i ) {
					if (i < timeFieldIndices[entity.third]) 	indexTransformation.put(i + indexShift, i);
					if (i == timeFieldIndices[entity.third])	continue;
					if (i > timeFieldIndices[entity.third])		indexTransformation.put(i + indexShift -1, i);
				}
			}
			return indexTransformation;
		}
	...
	private class CurPrevIndex extends Triple<PosRec,PosRec,Integer> {
		...
		public boolean equals(Object obj) {
			return obj != null &&
			obj.getClass() == CurPrevIndex.class &&
			((CurPrevIndex)obj).first.equals(this.first) &&
			((CurPrevIndex)obj).second.equals(this.second) &&
			((CurPrevIndex)obj).third.equals(this.third);
